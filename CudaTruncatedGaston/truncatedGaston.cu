// truncated differential distinguisher check for gaston
...
__global__ void gpuGastonDiff(uint64_t IV[], uint64_t key[], uint64_t nonce[], int64_t counter[], int round, int64_t trial) {
	...
	// truncated output masks
	uint64_t mask0 = 0x0000800000000000ULL;
	uint64_t mask1 = 0x0000000000000000ULL;
	uint64_t mask2 = 0x0000000000000000ULL;
	uint64_t mask3 = 0x0000000000000000ULL;
	uint64_t mask4 = 0x0000800000000000ULL;

	// expected values
	uint64_t expected0 = 0x0000000000000000ULL;
	uint64_t expected1 = 0x0000000000000000ULL;
	uint64_t expected2 = 0x0000000000000000ULL;
	uint64_t expected3 = 0x0000000000000000ULL;
	uint64_t expected4 = 0x0000800000000000ULL;

// Example: 
//The corresponding mask has 1s wherever your pattern has 0 or 1:
//00000001000000100000000000000000000000000101010000000000000100
//And the expected value has 1 only where your original pattern contains 1:
//00000000000000100000000000000000000000000000010000000000000000

	for (int64_t c = 0; c < trial; c++) {
		// Initial Difference
		pair0 = initial0 ^ 0x2000000000000000ULL;
		pair1 = initial1 ^ 0x0000000000000000ULL;
		pair2 = initial2 ^ 0x0000000000000000ULL;
		pair3 = initial3 ^ 0x0000000000000000ULL;
        ...
        // truncated masks
		uint64_t d0 = pair0 ^ initial0;
		uint64_t d1 = pair1 ^ initial1;
		uint64_t d2 = pair2 ^ initial2;
		uint64_t d3 = pair3 ^ initial3;
		uint64_t d4 = pair4 ^ initial4;
...
// #######################  GASTON TRUNCATED DIFFERENTIAL EXPERIMENT - MAIN ######################

long double gaston_truncated(int64_t experiment, int round, int64_t trial) {
	...
	if (total_counter == experiment)
		printf("Total counter: %lld, Probability: 1\n\n",
			total_counter);
...
void show_menu() {
	printf(">>> Gaston GPU-Accelerated Truncated Differential Distinguisher Verifier <<<\n\n"
		"(1) Truncated Differential Distinguisher\n"
		"(2) Clear screen\n"
		"(3) Exit\n\n"
		"Choice: ");
}
...
int main(void) {
	...
	while (1) {
		...
		if (choice == 1) {
			...
			gaston_truncated(experiment, round, trial);
		}
